...
const sections = ["立論", "反対尋問", "反対尋問", "立論", "反駁", "最終弁論", "反駁", "最終弁論"];
		...
		const getData = async () => {
			try {
					...
					const message =
						count === 0
							...
							: `${position2}側の意見です。${position1}派として500字以内で${
									sections[count]
							  }してください。発言回数は残り${4 - s1.comments.length}回です。
              ${s2.comments[s2.comments.length - 1]}`;
					...
					const message =
						count === 1
							? `あなたはディベートの参加者を演じてください。今回のテーマは「${agenda}」についてです。あなたは${position2}側として口語っぽく意見してください。あなたは4回発言ができ、そのうち最初の1回は500文字以内で${position2}側として反対尋問してださい。反対尋問は${position1}側の現状分析・${position1}側の問題点は重大なものか・問題解決はするか・実行可能か・本当にメリットは発生するのか、を元に行ってください。以下が${position1}派の意見です。
              ${s1.comments[s1.comments.length - 1]}
              「${position2}派の私としては、」から始めてください`
							: `${position1}側の意見です。${position2}派として500字以内で${
									sections[count]
							  }してください。発言回数は残り${4 - s2.comments.length}回です。
          			${s1.comments[s1.comments.length - 1]}`;
					const result: Response = await fetcher(
						"/api/generative-ai",
						conversationId,
