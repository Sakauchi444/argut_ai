...
const fetcher = async (url: string, conversationId: string, chatCodeId: string, message: string, model: string, position: string, section: string) => {
	const bot = bots[model as Model];
    ...
    const getData = async () => {
      try {
        if (caseSpeaker1.includes(count)) {
          const message = count === 0 ? `あなたはディベートの参加者を演じてください。今回のテーマは「${agenda}」についてです。あなたは「${position1}」側として意見してください。あなたは三回発言ができ、そのうち最初の一回は500文字以内で${position1}側として立論してださい。立論は現状分析・現状における問題点・問題解決のためのプラン・プラン導入後のメリット・デメリットを元に行ってください。` :
          `${position2}側の意見です。500字以内で反論してください。
          ${s2.comments[Math.floor(count / 2)]}`
          const result: Response = await fetcher("/api/generative-ai", conversationId, speaker1.chatCodeId, message, model1, position1, String(count+1));
          if (result.chatCode === speaker1.chatCodeId) {
            // IDが前回と同じ場合、contentを配列に追加
            setData({ ...data, speaker1: { ...s1, comments: [...s1.comments, result.message] }});
          } else {
            // 異なるIDの場合、配列を初期化して新しいcontentを追加
            setData({ ...data, speaker1: { ...s1, comments: [result.message] }});
            setSpeaker1({ ...speaker1, chatCodeId: result.chatCode });
          }
        } else {
          const message = count === 2 ? `あなたはディベートの参加者を演じてください。今回のテーマは「${agenda}」についてです。あなたは「${position2}」側として意見してください。あなたは三回発言ができ、そのうち最初の一回は500文字以内で${position2}側として立論してださい。立論は現状分析・現状における問題点・問題解決のためのプラン・プラン導入後のメリット・デメリットを元に行ってください。` :
          `あなたはディベートの参加者を演じてください。今回のテーマは「${agenda}」についてです。あなたは「${position2}」側として意見してください。${position1}側の意見です。500字以内で反論してください。
          ${s1.comments[Math.floor(count / 2)]}`
          const result: Response = await fetcher("/api/generative-ai", conversationId, speaker2.chatCodeId, message, model2, position2, String(count+1));
